...
SLASH_AERO1 = "/aero"
SlashCmdList["AERO"] = function(printInput)
    local newDuration = tonumber(printInput)
...
Aero:SetScript("OnUpdate", function()
    for i, frame in next, activeFrames do
        ...
        if aero.elapsed >= duration then
            ...
            local alpha = math.max(0, math.min(1,
                (scalePct <= 0.1 and 0) or
                (scalePct <= 0.8 and ((scalePct - 0.1) / 0.7) * 0.2) or
                (scalePct <= 1 and 0.2 + ((scalePct - 0.8) / 0.2) * 0.8)
            ))
...
function Aero:RegisterFrames(...)
    for i = 1, arg.n do
        ...
        if type(currentArg) == "string" then
            ...
            if not frame then
                return --print("Aero: " .. currentArg .. " not found.")
            end
